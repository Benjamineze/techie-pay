...
def show_predict_pg():
    ...
    if ok:
        user_input = np.array([[DevType, Country, Employment, OrgSize, WorkExp, Worktype, YearsCodePro]])
           
        user_input[:, 3] = np.vectorize(lambda x: variable_mappings['OrgSize'].get(x, 0), otypes=[int])(user_input[:, 3])
        user_input[:, 4] = np.vectorize(lambda x: variable_mappings['WorkExp'].get(x, 0), otypes=[int])(user_input[:, 4])
